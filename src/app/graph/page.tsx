...
export default function GraphPage() {
  ...
  const nodeComponentIdById = useMemo(() => {
    const nodeIds = new Set<string>((forceGraphData.nodes as any[]).map((n) => n.id));

    const adj = new Map<string, string[]>();
    for (const id of nodeIds) adj.set(id, []);

    for (const l of forceGraphData.links as any[]) {
      const sourceId = typeof l.source === "object" ? (l.source as any).id : l.source;
      const targetId = typeof l.target === "object" ? (l.target as any).id : l.target;
    ...
    for (const start of nodeIds) {
      ...
      while (stack.length) {
        ...
        for (const nb of neighbors) {
          if (componentById.has(nb)) continue;
          componentById.set(nb, componentId);
          stack.push(nb);
        }
      }
    }
  ...
  const componentAnchors = useMemo(() => {
    ...
    for (const [, cid] of nodeComponentIdById) componentIds.add(cid);

    const ids = Array.from(componentIds).sort((a, b) => a - b);
    const count = ids.length;
    if (!count) return new Map<number, { x: number; y: number }>();

    const cols = Math.ceil(Math.sqrt(count));
    const rows = Math.ceil(count / cols);

    const spacingX = Math.max(600, dimensions.width * 0.9);
    const spacingY = Math.max(600, dimensions.height * 0.9);
    ...
    ids.forEach((cid, i) => {
      ...
      const x = (col - (cols - 1) / 2) * spacingX + centerX;
      const y = (row - (rows - 1) / 2) * spacingY + centerY;
      anchors.set(cid, { x, y });
    });

    return anchors;
  }, [nodeComponentIdById, dimensions.height, dimensions.width]);
    ...
    (async () => {
      try {
        ...
        const collide = d3
          .forceCollide()
          .radius((n: any) => {
            const val = typeof n.val === "number" ? n.val : 5;
            // Conservative collision radius to prevent visual overlap of separate islands.
            return Math.sqrt(Math.max(0, val)) * 6 + 6;
          })
          .iterations(1);

        const fx = d3.forceX((n: any) => {
          const cid = nodeComponentIdById.get(n.id) || 1;
          return componentAnchors.get(cid)?.x ?? 0;
        });

        const fy = d3.forceY((n: any) => {
          const cid = nodeComponentIdById.get(n.id) || 1;
          return componentAnchors.get(cid)?.y ?? 0;
        });

        // Keep islands separated but not rigid.
        fx.strength(0.05);
        fy.strength(0.05);

        graphRef.current.d3Force("collide", collide);
        graphRef.current.d3Force("x", fx);
        graphRef.current.d3Force("y", fy);

        graphRef.current.d3ReheatSimulation();
      } catch {
        // If d3-force isn't available for some reason, keep the default simulation.
      }
    })();
  ...
  }, [componentAnchors, dimensions.height, dimensions.width, forceGraphData.nodes.length, nodeComponentIdById]);
